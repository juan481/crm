...
// Copia adaptada de src/components/directorio/empresa-notas.tsx para Deal —
// duplicado a propósito en vez de generalizado, ver Fase 8 del plan: es más
// seguro que arriesgar una regresión en la feature de Empresa ya en producción.
...
const TIPO_META: Record<Tipo, { label: string; icon: React.ReactNode; color: string; bg: string; placeholder: string; defaultMinutes: number }> = {
  NOTA: {
    ...
    placeholder: 'Nota sobre la oportunidad, observación, seguimiento...', defaultMinutes: 0,
  },
  LLAMADA: {
...
export function DealNotas({ dealId }: Props) {
  ...
  const { data, isLoading } = useQuery<{ data: DealNotaItem[] }>({
    queryKey: ['deal-notas', dealId],
    queryFn: async () => {
      const res = await fetch(`/api/deals/${dealId}/notas`)
  ...
  const notas: DealNotaItem[] = data?.data ?? []

  const handleDelete = async (notaId: string) => {
    if (!confirm('¿Eliminar esta nota?')) return
    setDeleting(notaId)
    try {
      const res = await fetch(`/api/deals/${dealId}/notas/${notaId}`, { method: 'DELETE' })
      if (!res.ok) {
        const json = await res.json()
        toast.error(json.error ?? 'Error al eliminar')
        return
      }
      qc.setQueryData<{ data: DealNotaItem[] }>(
        ['deal-notas', dealId],
