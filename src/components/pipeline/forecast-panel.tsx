...
//
// Multi-moneda (2026-09-24, reportado por Abba): antes sólo se proyectaba
// UNA moneda (la de mayor monto acumulado) y el resto quedaba afuera del
// todo, mencionado sólo en una línea gris al pie — un vendedor con
// oportunidades en USD y ARS literalmente no veía la mitad de su Pipeline
// acá. Ahora se arma una tira completa POR CADA moneda presente.
...
function buildModel(deals: Deal[], currency: string): CurrencyModel {
  ...
  return { currency, buckets, totalPonderado, totalGanado, max, sinFecha, sinFechaCount }
}
...
export function ForecastPanel({ deals }: { deals: Deal[] }) {
  const models = useMemo(() => {
    ...
    const currencies = Object.keys(byCur).sort((a, b) => byCur[b] - byCur[a])
    if (currencies.length === 0) currencies.push('ARS')
    return currencies.map((c) => buildModel(deals, c))
  }, [deals])

  const nothingToShow = models.every((m) => m.totalPonderado === 0 && m.sinFecha === 0)
  ...
  return (
    ...
    >
      ...
      {nothingToShow ? (
        ...
        models.map((model) => {
          const { currency, buckets, totalPonderado, totalGanado, max, sinFecha, sinFechaCount } = model
          if (totalPonderado === 0 && sinFecha === 0) return null
          return (
            <div key={currency} className={models.length > 1 ? 'pt-4 border-t' : ''} style={models.length > 1 ? { borderColor: 'var(--color-border)' } : {}}>
              <div className="flex flex-wrap items-end justify-between gap-3 mb-3">
                {models.length > 1 && (
                  <span className="text-xs font-bold px-2 py-0.5 rounded-full" style={{ background: 'var(--color-surface-raised)', color: 'var(--color-text)' }}>
                    {currency}
                  </span>
                )}
