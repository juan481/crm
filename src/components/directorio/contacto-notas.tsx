...
const TIPO_META: Record<Tipo, { label: string; icon: React.ReactNode; color: string; bg: string; placeholder: string; defaultMinutes: number }> = {
  NOTA: {
    ...
    placeholder: 'Nota sobre el contacto, observación, seguimiento...', defaultMinutes: 0,
  },
  LLAMADA: {
...
export function ContactoNotas({ contactoId }: Props) {
  ...
  const { data, isLoading } = useQuery<{ data: ContactoNotaItem[] }>({
    queryKey: ['contacto-notas', contactoId],
    queryFn: async () => {
      const res = await fetch(`/api/contactos/${contactoId}/notas`)
  ...
  const notas: ContactoNotaItem[] = data?.data ?? []

  const handleDelete = async (notaId: string) => {
    if (!confirm('¿Eliminar esta nota?')) return
    setDeleting(notaId)
    try {
      const res = await fetch(`/api/contactos/${contactoId}/notas/${notaId}`, { method: 'DELETE' })
      if (!res.ok) {
        const json = await res.json()
        toast.error(json.error ?? 'Error al eliminar')
        return
      }
      qc.setQueryData<{ data: ContactoNotaItem[] }>(
        ['contacto-notas', contactoId],
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const optimistic: ContactoNotaItem = {
      id:               optimisticId,
      tipo,
      content:          text,
