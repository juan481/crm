...
import { Plus, Pencil, Trash2, Package, Upload, CheckCircle, XCircle, AlertTriangle, Boxes, ArrowDownCircle, ArrowUpCircle, SlidersHorizontal } from 'lucide-react'
...
// Los productos "simples" cargados a mano (sin SKU) — cámaras, kits,
// instalación, lo que sea que Abba venda sin que venga del catálogo del
// proveedor. Antes vivía solo, en /configuracion/productos; ahora es una
// sección más dentro de Configuración → Catálogo, pestaña Productos, junto
// al catálogo del proveedor (SupplierCatalogManager) — misma lógica de
// siempre, sin cambios funcionales, sólo movida de lugar.
export function SimpleProductsManager() {
  ...
  const { data, isLoading, isError } = useQuery<{ data: Product[] }>({
    // scope=simple — el catálogo del proveedor (sku != null) lo administra
    // SupplierCatalogManager, acá sólo el puñado cargado a mano.
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h2 className="text-sm font-semibold uppercase tracking-wide" style={{ color: 'var(--color-text-subtle)' }}>Tus productos</h2>
          <p className="text-xs mt-0.5" style={{ color: 'var(--color-text-muted)' }}>Cargados a mano — cámaras, kits, instalación, lo que no venga del catálogo del proveedor</p>
        </div>
        {canManage && (
