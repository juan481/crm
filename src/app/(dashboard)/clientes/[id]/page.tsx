...
import {
  ...
  DollarSign, Target, CheckSquare, Clock, Mail,
} from 'lucide-react'
...
import type { Empresa, Deal, DealStage, Ticket, Task } from '@/types'
    ...
    <div className="space-y-5">
      ...
      {tab === 'tareas' && (
        <div className="space-y-2">
          {tareas.length === 0 ? <p className="text-sm text-center py-8" style={{ color: 'var(--color-text-muted)' }}>Sin tareas</p>
          : tareas.map(t => (
            <div key={t.id} className="rounded-xl px-4 py-3 flex items-center gap-3"
              style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', opacity: t.status==='HECHA'?.6:1 }}>
              <CheckSquare size={16} style={{ color: t.status==='HECHA'?'#10b981':'var(--color-text-muted)', flexShrink:0 }} />
              <div className="flex-1 min-w-0">
                <p className={`text-sm font-medium ${t.status==='HECHA'?'line-through':''}`} style={{ color: 'var(--color-text)' }}>{t.title}</p>
                {t.dueDate && <p className="text-xs flex items-center gap-1 mt-0.5" style={{ color: 'var(--color-text-muted)' }}><Clock size={10} />{formatDate(t.dueDate)}</p>}
              </div>
              <Badge variant={t.priority==='URGENTE'?'danger':t.priority==='ALTA'?'warning':'neutral'} size="sm">{t.priority}</Badge>
              {t.assignedTo && <span className="text-xs" style={{ color: 'var(--color-text-muted)' }}>{t.assignedTo.name}</span>}
            </div>
          ))}
        </div>
      )}
