    ...
    <div className="space-y-6 max-w-2xl">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-[var(--color-text-muted)]">Configurá tu servidor SMTP para enviar campañas</p>
        </div>
      </div>
      ...
      <div className="surface rounded-2xl p-4 border-l-4 border-blue-500 bg-blue-500/5">
        <div className="flex gap-3">
          ...
          <div className="text-sm text-blue-300 space-y-1">
            ...
            <p>Para Gmail: activá la verificación en 2 pasos → buscá &quot;Contraseñas de aplicación&quot; en tu cuenta de Google → generá una para &quot;Otra aplicación&quot;.</p>
          </div>
        </div>
      </div>
      ...
      <Card>
        ...
        <div className="flex flex-wrap gap-2">
          {PROVIDERS.map((p) => (
            <button key={p.label} type="button" onClick={() => applyProvider(p)}
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-3 gap-3">
              <div className="col-span-2">
                <Input label="Host SMTP" placeholder="smtp.gmail.com" error={errors.smtpHost?.message} {...register('smtpHost')} />
              </div>
              <Input label="Puerto" type="number" placeholder="587" error={errors.smtpPort?.message} {...register('smtpPort')} />
            </div>
            <Input label="Email / Usuario" type="email" placeholder="tu@email.com" error={errors.smtpUser?.message} {...register('smtpUser')} />
            <div className="relative">
              <Input
                label={hasPassword ? 'Contraseña (vacío = mantener actual)' : 'Contraseña / API Key'}
                type={showPass ? 'text' : 'password'}
                placeholder={hasPassword ? '••••••••••••' : 'Contraseña de aplicación'}
                {...register('smtpPass')}
              />
              <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-3 bottom-3 text-[var(--color-text-subtle)] hover:text-[var(--color-text)]">
                {showPass ? <EyeOff size={15} /> : <Eye size={15} />}
              </button>
            </div>
            <Input label="Nombre del remitente (opcional)" placeholder="Mi Agencia <hola@miagencia.com>" {...register('smtpFrom')} />
            {testResult && (
              <div className={`flex items-center gap-2 p-3 rounded-xl text-sm ${testResult.ok ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'}`}>
                {testResult.ok ? <><CheckCircle size={14} /> Conexión exitosa</> : <><XCircle size={14} /> Error: {testResult.error}</>}
              </div>
            )}
          </div>
        </Card>
        <div className="flex justify-end gap-3 mt-4">
          <Button type="button" variant="secondary" onClick={handleTest} loading={testing}>Probar conexión</Button>
