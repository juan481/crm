    ...
    <div className="space-y-6">
      ...
      <Modal open={action === 'password' && !!actionUser} onClose={() => { setActionUser(null); setAction(null) }} title="Cambiar Contraseña" size="sm">
        ...
        <ModalFooter className="mt-4">
          ...
          <Button onClick={() => updateUser(actionUser!.id, { newPassword, forcePasswordChange: true })} disabled={newPassword.length < 8}>Actualizar</Button>
        </ModalFooter>
      </Modal>

      <Modal open={action === 'suspend' && !!actionUser} onClose={() => { setActionUser(null); setAction(null) }} title={actionUser?.status === 'ACTIVE' ? 'Suspender Usuario' : 'Reactivar Usuario'} size="sm">
        <p className="text-sm text-[var(--color-text-muted)] mb-4">
          {actionUser?.status === 'ACTIVE' ? `¿Suspender a ${actionUser?.name}?` : `¿Reactivar a ${actionUser?.name}?`}
        </p>
        <ModalFooter>
          <Button variant="ghost" onClick={() => setActionUser(null)}>Cancelar</Button>
          <Button variant={actionUser?.status === 'ACTIVE' ? 'danger' : 'success'} onClick={() => updateUser(actionUser!.id, { status: actionUser?.status === 'ACTIVE' ? 'SUSPENDED' : 'ACTIVE' })}>
            {actionUser?.status === 'ACTIVE' ? 'Suspender' : 'Reactivar'}
          </Button>
        </ModalFooter>
      </Modal>

      <Modal open={action === 'delete' && !!actionUser} onClose={() => { setActionUser(null); setAction(null) }} title="Eliminar Usuario" size="sm">
        <p className="text-sm text-[var(--color-text-muted)] mb-4">
